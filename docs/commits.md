...
Note on the two less-common types:

- `style` — pure formatting (whitespace, imports order). Prefer
  folding into the next `refactor` / `feat` commit; use `style:`
  standalone only for tool-driven mass formatting.
- `revert` — usually generated by `git revert`. If you write one by
  hand, reference the SHA being reverted in the body.
